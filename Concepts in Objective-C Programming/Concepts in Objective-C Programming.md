...
NSObject 为初始化声明了 init 的原型；这是个实例方法类型，返回一个 id 类型的对象。对于需要对额外数据进行初始化对象的子类，重载 init 方法是很有必要的。不过，初始化一般要依赖于额外的数据来讲一个对象设置成一个合理的初始化状态。举例来说，假设你有一个 Account 类；要初始化一个 Account 对象通常需要一个唯一的账户号，并且它必须将其提供给初始化方法。因此初始化方法就需要提供一个或更多的参数；唯一必须的是初始化方法必须以 init 开头。（从代码风格上init... 通常代表初始化。 ） 
...
视图对象更倾向于复用和可配置，它们在应用程序之间保证一致性。在Cocoa中，AppKit框架定义了大量的视图对象并在界面构建库当中提供了多种选择。通过使用AppKit的视图对象，比如NSButton对象，你就能保证你的应用程序中的按钮的行为与其他Cocoa应用程序中的按钮的行为一致，确保跨应用程序的高度一致性。  
一个试图应该保证它所展示的模型的正确性。因此，通常需要知道模型的变化。因为模型对象不应该绑定在特定的视图对象上，它们需要一种通用的方式来指示它们已经更改了。
...
## Cocoa控制器对象的类型
	...
	iOS 注释：AppKit实现了NSController类和它的子类。这些类和绑定的技术在iOS上并不可用.

	深度阅读：更多关于Cocoa绑定技术，参见《Cocoa 绑定编程话题》
	
## MVC作为一个复合设计模式

模型-试图-控制器是一种结合了若干种基本设计模式的设计模式。这些基本模式互相结合定义了功能分层和。  
在原始的（Smalltalk）概念中，MVC 由复合、策略和监听模式组成。  

* 复合——视图对象在一个应用程序中是
* 策略——控制器对象会为一个或多个视图对象实现策略
* 监听——模型对象将感兴趣的对象保留在应用程序中——通常是视图对象——通知其状态的变化。

传统的复合、策略和监听模式互相协作已由图7-1 展示：
...
* 虽然你可以将 MVC 的角色全糅合进一个对象中，但是最好的策略是保持角色之间的分离。这种分离能够促进对象的可重用性，并对使用它们的程序的扩展性也有帮助。如果你将 MVC 的所有角色都合并到一个类里，
*  设计良好的MVC 应用程序应该使用尽可能多的（从理论上来说）可重用的对象。
* 
...
接收器设计模式是用来解决应用程序执行上下文中发生的事件重定向到另一个执行上下文以进行处理的一般问题。这是一个混合模式。虽然它没有出现在GOF中，但它结合了该书中命令模式、备忘录模式和代理设计模式的元素。这也是蹦床模式的变体（该书也未出现）；在此模式中，事件最初由蹦床对象接收，即所谓的事件，因为它会立即将事件退回或重定向到目标对象进行处理。
...
当你需要将任务退回到另一个执行上下文进行处理时，你可以采用接收器设计模式。当你监听一个通知或者实现一个回调处理，或者响应一个事件消息时，你要确保你的代码执行在一个合适的环境中，
...
目标是一种动作消息的接收者。一个控件或者更常见的说法，它将动作消息处理为一个接口(参见Outlets)。目标通常是一个你自己的类的实例，
...
UIKit 框架也声明和实现了一系列的控件类；这些框架中的控件类继承自 UIControl 类，此类定义了iOS的大部分的目标-动作机制。不过AppKit和UIKit框架中的实现目标-动作的机制从原理上还是有一些不同的。其中一个不同是UIKit没有任何真实的元素类。UIKit中的控件不依赖于它们的单元来提供目标和动作信息。  
一个更大的不同是两个框架如何实现目标-动作的事件模型的性质。在AppKit框架当中，用户通常使用鼠标和键盘来注册被系统处理的事件。这些事件——比如点击一个按钮-是被限制和分离的。

与AppKit框架相比，一个动作方法可能只有一个或者两个有效的签名，而UIKit框架允许三个不同形式的动作选择器：  
