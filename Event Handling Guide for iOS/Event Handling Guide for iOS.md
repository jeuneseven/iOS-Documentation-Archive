...
### 与其它的手势识别进行交互
一个视图可以有多个手势识别附加。使用视图的gestureRecognizers属性来判断是什么手势添加到了视图上了。你还可以通过从视图添加（addGestureRecognizer:方法）或移除（removeGestureRecognizer:方法）手势识别来动态的改变视图处理手势的方法。  
当一个视图有多个手势识别附加的时候，你可能就需要改变冲突的手势识别接收，并分析触摸事件了。默认是没有哪个手势识别是触摸事件的第一接受者的，由于这个原因，触摸事件可以被以不同的顺序传递给手势识别。你可以重写这一默认行为来：  

* 指定一个手势识别在其他手势识别之前来解析触摸事件。
* 允许两个手势识别来同时操作。
* 阻止手势识别来解析触摸事件。

你需要使用UIGestureRecognizer类的方法，代理方法以及被其子类所重写的方法来影响这些行为。
...
# 事件传递：响应者链条
当你设计你的应用程序时，很可能你希望你的应用程序能够动态的响应事件。举例来说，触摸事件可能发生在很多种不同的对象的屏幕上，你需要判断哪个对象来响应事件并理解对象是如何接收事件的。  
