...
更多关于 macOS 上的授权和鉴权的概念，阅读《授权，鉴权和许可指南》。  
你可以在《其他安全资源》中了解更多其他的苹果和第三方安全书籍。
...
本章描述了这些加密技术并简单概述了 macOS 和 iOS 提供的用来帮你在你自己的应用中使用加密技术。
...
```
注意：尽管两个密钥从数学上相关，但从一个密钥派生另一个密钥在计算上被认为是不可行的。公钥密码的安全性就取决于这种情况。
```
...
非对称加密通常用来建立一个共享沟通渠道。由于非对称加密的计算昂贵，两个末端通常使用非对称加密交换对称密钥，然后使用较快的对称加密算法来加密解密实际的数据。  
非对称加密也可以背用来建立互信。通过用你的私钥加密信息，其他人就可以通过你的公钥确定阅读你所加密的信息了。
...
iOS 提供了 APIs 来允许一个应用仅当设备解锁的时候创建、访问文件，保护它们的内容。为了数据安全，文件以加密格式存储并且仅在用户输入密码后才解密。

## 了解更多 

更多关于 macOS 上的授权鉴权的详细概念，阅读《加密服务指南》。  
你还可以在《其他安全资源》中了解更多其他苹果和第三方安全书籍。
...
* 苹果的开放库网站 提供了开源安全代码。你可以查阅它来查找苹果的 macOS 和 iOS 安全实现锁支持的安全协议和算法，并查询额外的文档。
* macOS 开发者库以及 iOS 开发者库的安全主题区包含大量的安全相关的发布记录。
