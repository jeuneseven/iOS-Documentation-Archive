...
# 为性能而开发
性能是软件设计的一个经常被忽视的方面，直到它变成很严重的问题时才被重视。如果你在开发周期快要结束时才开始进行性能调试的话，就很可能做不到任何有意义的调试了。性能是在早期设计阶段当中就应当被包含进去的，并且是贯穿整个开发周期的。  
...
### 要懒
一个非常简单的提升性能的方式是确保你的应用程序没有执行任何不需要的工作。应用程序的所有时间都应该花在响应用户当前的请求上，而非预测未来的请求。如果你并不是立刻就需要某个资源的话，比如一个包含偏好设置窗口的nib文件，那就不要加载它。比如一个访问文件系统的动作执行时比较耗时，并且用户根本不会打开偏好设置窗口，那么加载该nib文件就是浪费时间了。  
基本原则是要遵循等待用户请求你的应用程序时再去使用必要的资源来填充该请求。你应当只在比较重要的有影响性能的情况下才缓存数据。如果假定应用程序的其余部分运行得更快, 则预加载缓存实际上会降低低内存情况下的性能。在这种情况下，你的缓存数据可能会在其被使用之前被分页到磁盘上。因此，通过缓存数据获得的任何节省都会变成损失，因为必须先从磁盘读取两次数据，然后才能使用它。如果你真的想要缓存数据的话，要在操作执行一次之后再这么做。  
其他的懒加载的内容包含以下几点：  

* 推迟内存分配直到你真正需要内存的时候。
* 不要零初始化内存块。调用calloc函数来代你懒加载。
* 给予系统机会来懒加载你的代码。描绘和组织你的代码以便系统只在需要当前操作的时候才加载代码。
* 推迟读取文件的内容，直到你实际需要该文件的信息为止。

### 利用已知的性能数据
在很多情况下，对于性能的看法与实际性能一样有效。很多任务都可以在后台执行，使用调度队列或在闲置时间。这么做能够让应用程序的主线程空闲出来处理用户交互并且让程序的界面对于用户而言响应的更多。在你设计你的程序的时候，尽量考虑一下哪些任务能够移到后台高效执行。举例来说，如果你的程序需要扫描一组文件或者执行长度计算的话，那么使用调度队列这么做吧。  
另一个能够提高性能感知的方法是确保你的应用程序启动的够快。在启动时，推迟任何对于你的应用程序界面迅速展示没有贡献的任务。比如，推迟大的数据结构的创建，直到你的应用程序已经完成加载并展示主窗口之后。如果你要在主窗口展示一些必须计算的数据或者要在启动时恢复的话，要在展示窗口的时候展示一个进度指示或者其他的能够标识数据正在被加载的状态信息。对于使用插件的应用程序，要在插件代码真正用到的时候再加载。
### 使用基于事件的处理
所有的现代Mac应用程序都应该使用Cocoa事件系统或者Carbon事件管理。（同样的，iPhone应用程序必须使用基于触摸的事件系统，该事件系统由UIKit框架提供）你的应用程序无需轮询查询系统来检索事件。这么做是非常低效的。实际上，当没有事件在处理的时候，轮询代码是百分百浪费CPU时间的。现在基于事件的APIs被设计为提供了以下优势：  

* 它们能够让你的程序更多的响应用户。
* 它们能够降低你的应用程序的CPU使用率。
* 它们能够最小化你的应用程序的工作单元——即在任意给定时间加载到内存中的代码页数。
* 它们能够让系统更积极的管理电源。

除了用户事件，你也应该避免在其他情况下轮询。在OSX和iOS当中，线程是使用的run loop来提供命令响应给timer，网络事件和其他的数据的。很多框架对于一些特定任务也使用了异步编程模型，这会在一个任务完成时通知一个已经设计好的回调函数或者方法。在OSX v10.6及以后的版本中，调度资源也为你提供了异步的接收重要事件的方法，你可以在一个调度队列中执行它们。  
### 提高你的程序任务的并发性
对于有着多核心的计算机，并发性是另一个提高你的程序的感知和性能的比较好的方法。能够在一个时间点内执行多个任务的程序也就能够在多核心的计算机上并行的执行这些任务。即使当计算机只有一个核心的时候，将你的代码分解为多核的，异步的任务也能够在正确的执行时提供更好的速度提高的感知。尤其是，你应该使用调度队列来执行自定义任务，让你的主线程空闲出来处理用户事件并更新用户界面。  
不过，在你开始添加并发性的支持之前，你要考虑一下你的程序如何有效的实现响应的任务。将你的代码分解为不同的任务需要你的程序数据结构和代码路径满足一些条件。共享数据结构的任务也可能需要使用串行调度队列来同步访问这些数据结构。  
有关如何在你的程序中实现并发的相关信息，参见“并发编程指南”一文。
### 使用加速framework
如果你的应用程序在标准数据上执行了大量的数学计算，你应该考虑使用Accelerate.framework来对这些计算进行加速。Accelerate framework利用了可用的向量处理单元（比如Intel的x86 SSE扩展）来并发的执行计算。通过执行框架代码来替代向量单元，你可以避免为每种平台架构创建不同的代码路径。Accelerate.framework对于OSX架构的支持是经过高度调试的。  
类似Instruments的工具能够帮助你找出使用Accelerate框架时的收益点。更多关于使用这些工具的相关信息，参见“性能工具”部分。
### 使你的应用程序更现代化
如果你的程序是被设计用来运行在较老版本的MacOS上的话，你应该更新你的代码来支持OSX的约定。尤其是你应该避免使用类似Carbon或者legacy 这种太老的技术。你应该使用cocoa或cocoa touch来构建你的应用程序。你也应该更新你的二进制格式为Mach-O。Mach-O格式是基于Intel的Mac机器和iOS设备唯一支持的格式。  
有关Mac应用程序中可用的技术列表，参见“Mac技术概览”。有关iOS中的可用技术列表，参见iOS技术概览。
# 性能工具
...
### Instruments
Instruments封装了一组强有力的分析工具，结合可视化的用户界面，提供了有关你的应用程序的各种运行时行为的相关信息。它并不是一次性的展示你的程序的一个方面，你可以为每个分析单元进行配置，每个单元都可以收集特定的性能标准的信息。所有的数据都由instruments一个接一个的展示出来，使得你的应用程序的行为趋势更为容易在instrument之间的数据上进行关联。  
你可以通过Instruments进行收集的数据类型包括以下方面：  

* 基于Core Data的应用程序性能数据
* 有关文件系统的读写和其他操作的信息
* 垃圾收集代码相关的统计
* 关于图形操作和性能的信息
* 关于对象和其他内存分配相关的统计
...
有关如何使用Instruments的简单例子，参见“使用Instruments”。有关详细信息，参见“Instruments用户指南”。

## 分析工具
Instruments并不是唯一的能够收集你的应用程序数据的分析工具。其他的分析工具旨在查找特定类型的性能问题。列表3-1列出了随Xcode安装的分析工具和可以下载的工具。  
...
OpenGL Profiler  | 为你的基于OpenGL的应用程序创建一个运行时分析。你可以直观的看到函数统计以及你的应用程序OpenGL调用的调用栈历史。该工具是Xcode可视化工具的一部分，在Apple开发者网站上的下载页面可以见到。
heap  | 列出所有指定进程的堆中的malloc-allocated缓冲区。该工具安装在/usr/bin/。
leaks  | 检索一个进程的内存空间中的申请但没有用到的内存块。该工具安装在/usr/bin/。
vmmap  | 展示一个指定进程的虚拟内存区域分配。你可以使用该工具来分析你的进程的内存使用。该工具安装在/usr/bin/。

## 监控工具
监控工具是一种自动的被动收集数据的工具。要使用这些工具，在你运行你的程序的功能时让其运行即可。你可以通过这些工具生成的数据来分析数据并更好的理解你的应用程序的特色。一些程序应该始终运行着。大部分其他的程序也能够根据需要来收集性能信息。列表3-2列出了OSX上的监控工具以及可以下载位置。  
...
Quartz Debug  | 通过短暂的闪动区域重绘来实时的展现屏幕的更新。你可以使用该工具来分析你的应用程序的绘制功能。该工具是Xcode可视化工具的一部分，在Apple开发者网站上的下载页面可以见到。
fs_usage  | 展示并列出当前文件系统的活动，通过分页错误生成以及调用文件系统函数。你可以使用该工具来理解你的程序的文件访问部分。该工具安装在/usr/bin/。
...
工具  | 描述
------------- | -------------
Reggie SE  | 能够让你检测和修改CPU和PCI配置注册。
acid  | 
simg4 | 
simg5 | 

## 额外的命令行工具
列表3-4列出了一些你在OSX中可以用来监控和分析性能的额外的命令行工具。（这些工具不能够运行在iOS上监控和分析应用程序。）这些工具存在/usr/bin/目录下，并且必须从命令行运行。大部分同Xcode工具一起安装。更多关于这些工具的信息，参见OSX Man页面。  

列表3-4 命令行工具

工具  | 描述
------------- | -------------
atos | 负责在符号表名和该符号在运行程序中的数字地址之间进行转换。
c2ph  | 从一个对象文件和其成员位移值中展示C结构体。
gprof | 
malloc_history | 
nm | 
otool | 
...
# 开始性能评估
现在，已经有了一些代码，你想要看看它是否受到性能的影响。你从何开始呢？并不是所有的问题都能够立竿见影的。你可能注意到一个操作消耗了几秒钟执行，但你可能没有注意到一个操作可能消耗了大量的CPU循环或分配了大量的内存。这时就是Apple的性能工具出场的时候了。它们能够帮助你检测你的程序，使得那些容易被忽略的部分让你看到。  
在你开始分析你的程序时，接下来的段落提供了一些如何使用一些关键工具的摘要。这些工具对于定位潜在的问题很有效并且提供了大量的性能数据的表征。记住，尽管如此，还有其他的工具关于问题提供了更为详细的信息。结合其他的工具来运行你的应用程序能够帮助你确定一个特定区域是否有问题。  

```
重要：性能工具是帮助你检查性能问题的。要在分析过程中尽可能确保收集足够多的数据。性能分析更像是一门艺术，需要仔细考虑所有可用的数据，以发现真正的问题。
```
更多关于常见性能工具的信息，包括如何获取他们，参见“性能工具”一节。
## 使用top
工具top是一个很重要的工具，它能够定位一个进程中的潜在问题区域。该工具展示了一个周期内系统使用的统计数据样本。使用top并理解其输出是一种很棒的定位潜在性能问题的方式。  
工具top展示了CPU使用，内存使用（不同的分类），资源使用（比如线程和端口）以及分页事件的定期更新统计。在默认的情况下，top展示的是整个系统进程的CPU和内存利用率。你可以使用这些信息分析你的程序使用了多少内存以及有多少比例的CPU时间被使用了。闲置的程序不应该使用任何CPU时间，而一个活跃的程序会根据其任务的复杂性消耗一定比例的CPU时间。
## 使用Instruments
Instruments是一个非常强大的工具，你可以用它来手机性能数据并分析你的APP的整体行为。Instruments可以展示给你不同类型的性能信息，比如内存和CPU使用，并且是图形化的一个个展示。以这种方式查看信息能够更简单直观的定位趋势和不同度量之间的关系。  
当你首次加载Instruments的时候，你需要从（图4-1）中选择一个开始的模板并选择一个APP来分析。
