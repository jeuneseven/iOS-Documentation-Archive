...
## 通过调试过程进行循环
调试是一个反复进行的过程。你发现了一个问题，设置断点来帮助你定位到底是源代码的哪部分出了问题，检验回溯和变量的评估状态以及引起问题的原因，然后你设计一个解决方案或修复它。有一个潜在的解决方案在手，你可以对源代码进行修改并重新运行应用程序来查看问题是否被解决。若检测发现问题依旧存在，你可以重复这一循环直到正确修复了问题。  
当问题解决后，你可以禁用或删除你在这次调查中使用的断点。这是因为你不希望它们带到下一个问题当中。
# 调试工具
