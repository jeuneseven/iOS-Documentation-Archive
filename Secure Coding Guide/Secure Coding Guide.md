...
本文档首先简要介绍了软件中常见的每种安全漏洞的性质。比如，如果你不确定条件判断是什么，或者它为什么会带来安全风险，则可以从《安全漏洞类型》一章开始。  
其他的章节详细解释了特定类型的安全威胁。这些章节可以以任意顺序阅读，或者以《安全开发检查表》中的软件开发检查表所建议的顺序阅读。  

- 《避免缓冲溢出和下溢》描述了缓冲溢出的几种类型并解释了如何避免。
- 《非法输入和进程间通信》讨论了你该如何验证每种从不可信任的源头到你的程序中的输入。
- 《种类条件和安全的文件操作》解释了种类条件是如何发生的，讨论了如何避免它，并描述了不安全和安全的文件操作。
- 《提高权限的安全性》描述了如何避免在高权限下运行代码以及如果你完全无法避免它的话该怎么做。
- 《设计安全的用户接口》讨论了一个程序的用户接口如何提高或妥协安全，以及在如何编写安全-提升的 UI 中给出指南。
...
缓冲溢出通常发生在当一个应用试图编写超过缓冲区尾部的数据（或者超过头部）。  
缓冲溢出会引起应用崩溃，会损害数据，并且在应用运行时提供一个攻击因子进一步的恶化权限来危害系统。  
关于软件安全的书籍总是提到缓冲区溢出是漏洞的主要来源。具体数据很难得到，2004 年美国计算机快速应急团队（US-CERT）报告大约 20% 的公开漏洞都包含缓冲区溢出问题。  
任何应用或系统软件都会从用户、文件或从网络存储获取输入。除了一些特殊情况，大部分应用内存都存储在一个或两个位置：  

- 栈——应用程序的地址空间的一部分，它存储的数据被特定函数，方法，block 或其他相同结构直接调用。
- 堆——应用主要的存储位置。只要程序在运行，存储在堆上的数据就是可用的。（或者直到应用直接告知操作系统不需要该数据了为止）

类的实体，被 malloc 声明的数据，核心框架对象以及其他应用存到堆中的数据。（不过，注意，局部变量实际上指向的是存储在栈上的数据。）

缓存溢出攻击通常通过堆、栈或这两者一起的侵入所发生。更多信息，阅读《避免缓冲溢出或下溢出》。

## 无效的输入

作为通用规则，你应该检查所有的通过你的程序所接收的输入，确保数据是有效的。  
比如，一个图像文件可以合理的包含一个 200 * 300 像素的图片，但包含一个 200 * -1 像素的图片就不合理了。但是，没有什么能走阻止一个文件声称包含一个这样的图片。实体程序试图读取这样的文件会尝试分配一个不正确大小的缓冲区，导致潜在的堆溢出攻击或者是其他程序。由于这个原因，你必须仔细检查你的输入数据。这个过程通常被叫做输入有效或正常性检查。  
任何你的程序从不可信源接收的输入都是潜在的攻击对象。（在这种环境下，一个普通的用户都是不可信源。）一个不可信源的典型输入包括（但并非严格包含）：
 
* 文本输入区域
* 通过一个 URL 传入的命令，用来启动程序
* 音频，视频或者由用户或其他进程提供的图像文件以及程序读取
* 命令行输入
* 通过网络从一个不可信的服务器读取的任何数据
* 通过网络从一个可信服务器读取的任何不可信数据（比如，用户提交的 HTML 或者 图片）



## 条件判断

更多关于条件判断的信息以及如何阻止它，阅读《条件判断和安全文件操作》。

## 进程间通信

更多关于如何执行安全的进程间通信，阅读《有效输入和进程间通信》。
...
无论是在数据传输还是数据存储，加密都能够被用来保护用户的隐私。（此处未解决如何保护供应商数据不被未经许可复制或使用的问题。）macOS 提供了一系列的基于加密的安全选项，比如：
...
更多关于如何设计用洁面增强安全的建议，参见《设计安全的用户界面》。
...
本文档的其他章节描述了编写安全编码的最佳实践，包括更多的上述所引用的主题的信息。  
《安全概览》以及《加密服务指南》包含 macOS 开发者可以使用的安全功能的详细信息。
