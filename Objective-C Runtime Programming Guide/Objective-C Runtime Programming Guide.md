...
转发的方法（被称作消息转发）
## 动态加载
OC程序是可以在运行时加载和链接一个新类和分类的。新的代码与当前程序合并，并和开始时的类和分类一样进行加载。  
动态加载可以被应用在很多不同的场合。举例来说，系统配置程序中的很多不同模块都是动态加载的。  
在Cocoa环境中，动态加载常被用来让应用程序定制化。其他的你的程序在运行时加载的能够编写的模块——比如界面构建器加载自定义调色板和OSX系统设置应用加载自定义配置模块。可以加载的模块能够扩展你的应用程序。他们以你允许的方式作出贡献，但无法预料到或定义本身。你提供framework，但是由其他人提供代码。  
虽然有运行时函数来在Mach-O文件中提供执行动态加载OC模块（objc_loadModules，定义在objc/objc-load.h中），Cocoa的NSBundle类还提供了一个更便捷的接口来给动态加载——该接口是面向对象的，并且与相关服务整合。参见Foundation 框架中的 NSBundle 类说明。查看"OS X ABI Mach-O File Format Reference"来了解Mach-O文件。
# 消息向上转发
给一个对象发送该对象无法处理的消息会造成错误。不过，在错误发生前，运行时系统会给接收消息的对象一次机会来处理消息。  
## 消息转发
...
# 声明属性
当编译器遇到属性声明时（参见《OC编程语言》中的“声明属性”部分），它会生成与封闭类、分类或协议相关的描述元数据。你可以使用函数访问这些元数据，这些函数支持按照类或者协议上的名称查找属性，将属性类型作为@encode字符串，以及将属性的属性列表复制为C字符串数组。声明的属性列表可用于每个类和协议。  
## 属性类型和函数
