...
BindingsJoystick
## 内存管理（Memory management）
内存管理是指管理对象的生命周期以及当对象不再被需要时释放的编程规范。管理对象的内存的关键是性能问题；若一款应用程序不释放不再使用的对象的话，那么它的内存会增加，性能将会受损。在Cocoa的应用程序中的内存管理不使用垃圾收集机制，而是基于引用计数模型。一个对象的拥有者同样要负责对象的释放，意思是要减少引用计数。当引用计数变为0时，对象被释放（销毁）。  
为了帮助你进行内存管理，OC提供了你必须使用的方法和机制, 以符合一组规则。  

```
注意：在OS X中，你可以指定使用内存管理或使用OC提供的垃圾回收功能。垃圾回收在iOS上是不可用的。
```

![](https://developer.apple.com/library/content/documentation/General/Conceptual/DevPedia-CocoaCore/Art/memory_management_2x.png)  

### 内存管理规定
内存管理规定有时候被称作“所有权协议”，帮助你在OC代码中明确的管理内存。  

* 你拥有通过分配内存创建或拷贝的对象。相关方法：alloc, allocWithZone:, copy, copyWithZone:, mutableCopy, mutableCopyWithZone:。
* 若你不是一个对象的创建者，但你想确保它是存在在内存中并为你所用的话，你可以通过表达式来确定。相关方法：retain。
* 若你拥有一个对象，无论是通过创建或持有，在你不需要它的时候，你都应该负责释放它。相关方法：release, autorelease。
* 相反的，若你不是一个对象的创建者或不持有该对象的话，你就不应该释放它。

若你在你的程序中从任意位置接收到了一个对象，通常需要在你的函数或方法中校验接收到的对象是否合法。若你想在作用域范围之外依旧让对象有效的话，你需要持有或拷贝它。若你试图释放一个已经被释放的对象的话，你的程序将会崩溃。
### 内存管理的各方面
以下概念对于理解和适当的使用内存管理很重要：  

* 自动释放池。给一个对象发送autorelease消息会将对象标记为稍后释放，当你需要将对象持有超过作用域范围之外再对其进行释放时很有用。自动释放一个对象会将对象放入一个自动释放池（一个 NSAutoreleasePool 的实例对象），它是为任意程序范围创建的。当程序执行完那段代码时，在池中的对象将会被释放。
* 重新分配地址。当一个对象的引用计数降至0时，运行时会在销毁该对象前调用对象的类的dealloc方法。实现该方法的类会释放任何与对象持有的资源，包括对象指向的自己的实例变量。
* 工厂方法。为了方便起见，很多框架的类都定义了类方法，你可以使用它来创建该类的对象。这些返回的对象是不能够保证在接收方法范围之外依旧有效的。
...
## 消息（Message）
消息是一个方法的名称以及与它相关的参数，它由对象发送和执行。若要让一个对象做某件事的话，你需要给它发送一个消息让它执行一个方法。在OC中，你需要指定对象（被称作方法的接收者）然后使用中括号将要发送给该对象的消息括起来。比如，若想让myRectangle对象执行display方法：  

	[myRectangle display];

(该表达式末尾紧跟一个分号，这与其他C的代码一样。)  
消息中的方法名用来选择一个方法的实现——当消息被发送的时候，运行时系统会从接收者的指令和调用中选择适当的方法然后发送给它。由于这个原因，消息中的方法名通常被称作selectors。  
方法也能够携带参数，被称作arguments。有一个参数的方法在方法名之后带有一个冒号(:)，冒号的右侧紧跟参数。这种结构被称作关键字结构；关键字后紧跟冒号，然后参数在冒号后。一个拥有多个参数的方法会有多个关键字，每个关键字后都紧跟冒号。
...
## 模型对象（Model object）
模型对象是一种包含应用程序数据的对象，提供访问该数据的能力，并提供修改该数据的逻辑实现。模型对象在MVC设计模式中扮演着其中一个角色。（其他两个角色由视图和控制器对象所扮演。）任何处于持续状态的应用程序的一部分数据都应当存在于数据被加载到应用程序之后的模型对象当中。  
由于模型对象代表着一个特定主题的相关信息，当该主题出现的时候，它们就能够被复用。在理想的情况下，一个模型对象应当与展现数据并允许用户编辑数据的视图对象没有明显的关系，换句话说——它不应当与用户界面以及展现有任何联系。

![](https://developer.apple.com/library/content/documentation/General/Conceptual/DevPedia-CocoaCore/Art/model_object_2x.png)

### 一个设计良好的模型类
一个模型类——指的是一个类生产模型对象——通常是NSObject对象的子类，若你使用Core Data技术的话，会是NSManagedObject对象的子类。若要创建一个模型对象的话，你需要将其定义为一个基本类并遵守Cocoa命名规范。比如，在实例变量名称的开头，声明属性和声明方法都以小写字母开头并在随后的单词的每个首字母大写。  
当你实现你的模型子类的时候，你应当在以下类的设计方面进行考虑：  

* 实例变量。你需要声明实例变量来持有应用程序的封装数据。实例变量可以是对象；基本类型或者NSRange这样的结构体。使用对象还是非对象的类型需要权衡，对象是否可变也是一个需要考虑的因素。
* 存取方法和声明属性。存取方法和声明属性为保存封装提供了方法，因为它们能够见解的访问一个对象的实体数据。存取方法通常获取和设置实例变量的值（通常被称作getter和setter方法）。生命属性是一个语言级别的便捷属性，它能够允许运行时为一个类合成存取方法。存取方法和声明属性一个很重要的作用是管理对象的内存。由于这个原因，实现getter和setter方法时是有固定的格式的。
* 键值编码。KVC是一种让客户端通过使用属性的名称访问对象的属性的机制。它被Core Data和Cocoa的很多地方所使用。存取方法的名称（当然包含声明属性）也是该机制的一个因素。
* 初始化和销毁。大部分情况下，一个模型类回实现一个初始化方法来将其实例变量设置为一个合适的初始值。它应当遵循初始化方法的标准格式。也应当在dealloc方法中释放实例变量所持有的对象的值。
* 对象编码。若你的模型类的对象需要被归档的话，要确保该类对于对象的实例变量进行了编解码。
* 对象拷贝。若你需要客户端拷贝你的模型对象的话，你的类应当事先对象拷贝。
...
交互：控制器对象将用户的行为进行解释，然后添加到视图对象当中，并将增加或改变的数据更新到模型层。当模型对象改变的时候，控制器对象会将新的模型对象传递给视图对象以便后者对其进行展示。
### 预读文章
消息（Message）
### 相关文章
模型对象（Model object）
...
模型-视图-控制器（Model-View-Controller）
## 多种初始化（Multiple initializers）
一个类可能定义多个初始化方法，无论是它能够为输入提供多种不同的格式还是提供默认的初始值为客户端提供便利。若一个类能够将初始化数据以不同的的形式或从不同处进行的话，它就能够为每个格式或源头声明不同的初始器。比如，NSString类拥有Unicode字符或从URL资源获取字符串作为数组的初始器。  
多个初始化方法还能为编程带来便利。类可以有一系列的初始值设定项, 一方面, 允许客户端指定所有初始值, 另一方面, 将大多数或所有这些值作为默认设置提供。  
框架类有时候也会拥有多个工厂方法作为初始化方法。这与提供便捷性或以不同的形式携带初始值类似。不过，它们分配内存并返回的对象也是初始化过的。

![](https://developer.apple.com/library/content/documentation/General/Conceptual/DevPedia-CocoaCore/Art/multiple_initializers_2x.png)

### 指定的初始化
一个类的初始器会拥有需要初始的所有参数，通常被称作“指定初始器”。一个子类的初始器必须调用通过给super发送消息来调用其父类的指定初始器。简便的初始器（或次一级）——可以包括init——无需调用super。相反, 他们在一系列初始化中使用较多的参数调用那一个 (通过消息到自己) 初始值设定项, 为未传递给它的参数提供默认值。此系列初始化方中的最终初始值设定是指定的初始值设定。
### 预读文章
对象的创建（Object creation)
初始化(Initialization）
### 相关文章
...
对象是动态创建的（Objects Are Created Dynamically）
## nib文件（Nib file）
nib文件是一种特殊类型的资源文件，你可以使用它来存储iOS和Mac的app的用户界面。一个nib文件是一个界面编辑器的文档。使用界面编辑器来构建app的可视化部分——比如窗口和视图——有时还能用来配置非可视化的对象，比如你的app用来管理窗口和视图的控制器对象。事实上，当你编辑一个界面编辑器文档时，在你保存该文件的时候你就创建了一个该对象的归档。当你加载该文件的时候，对象是被解档的。  
nib文件——因此也叫做对象的图表——可能包含占位符对象，用来指代不存在在文档当中的对象但对文档中的对象有引用关系，或文档中的对象所引用的对象。一种特殊的占位符叫做文件拥有者。  
在运行时，使用loadNibNamed:owner: 方法或它的变体来加载nib文件。文件的所有者是作为该方法的所有者参数传递的对象的nib文件中的占位符。无论你在界面编辑器的nib文件中的文件所有者上创建什么连接，在运行时加载文件的时候这些连接都会被重新建立。  
iOS使用nibs作为支持故事版的具体实现，它是iOS用户界面设计的布局规范。故事版让你能够在一块画布上设计和看到你的app的整个用户界面。对于iOS开发者而言，我们推荐使用故事版来设计用户界面。
### 预读文章
...
Reducer
## 对象关系（Object graph）
在面向对象的程序中，一组组的对象通过它们之间的联系构成了一个网络——无论是通过直接引用一个另外的对象或通过一个间接引用链。这些一组组的对象被称作对象关系。对象关系可小可大，可简单可复杂。一个包含单一字符串对象的数组对象代表了一个小的，简单的对象关系。一组包含应用程序对象，引用窗口，菜单和其他视图以及其他支持的对象的对象可能代表了一个大的复杂的对象关系。  
有时你可能需要转换对象关系——通常是在应用程序中的一组完整对象关系——保存到文件中或传输到其他进程或随后需要重新构建的机器中。这个过程被称作归档。  
某些对象关系可能不完整——这通常被称作局部对象关系。局部对象关系拥有占位对象，它代表关系的边界可能在随后的某个阶段被填补上。一个包含文件拥有者的nib文件就是一个很好的例子。
### 相关文章
对象的归档（Object archiving）
nib文件（Nib file） 
## 对象生命周期（Object life cycle）
