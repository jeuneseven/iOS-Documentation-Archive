...
对于容器类视图控制器，父类取决于你是在修改一个已经存在的容器类还是创建你自己的容器。对于已经存在的容器，你可以任意选择已经存在的容器类来修改。对于你创建的新的容器视图控制器，通常继承于UIViewController。关于创建一个容器视图控制器的相关信息，参见“实现一个容器类视图控制器”。
### 定义你的UI
...
## 构建和适配界面
一个适配的界面应当同时响应特性和尺寸的变更。在视图控制器层级，你使用特性来进行粗粒度的关于你展示的内容的决策以及对该内容进行布局。举例来说，当尺寸类发生改变时，你可能需要选择改变视图的属性，展现或隐藏视图或展示一组完全不同的视图。在这些重大决策做出之后，你应使用尺寸变更来对你的内容进行微调。
### 适应特性的改变
特性（Trait）给予了你一种方式以不同的方式来配置不同环境的应用程序，你使用它来进行粗粒度的调整界面即可。大部分你需要使用特性来做出改变的内容都可以直接在故事版文件中变更，不过有些需要额外的代码实现。
#### 配置你的故事版来处理不同的尺寸类
