...
装饰器对于对象的构成而言是一种模式，我们也建议你在你自己的代码中这么做（参见“何时该创建子类”）。不过，Cocoa本身提供了一些类和机制（在后续章节中讨论）是基于这种模式的。在这些实现中，扩展的对象不完全重复其封装的对象的接口，且其实现使用了不同的接口共享技术。  
Cocoa使用装饰模式在实现以下几个类当中，包括NSAttributedString, NSScrollView, 和 UIDatePicker。后面的两个类是两个复合视图的例子，它们将其他视图类的简单对象组合在一起并协调其相互作用。
