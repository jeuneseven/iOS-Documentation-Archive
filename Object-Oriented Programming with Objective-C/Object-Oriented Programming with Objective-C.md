...
举例来说，你可以将对象想象成演员，然后赋予它们类似人类的意图和能力。有趣的是，有时候可以与一个对象进行沟通关于一种情况下该如何做，询问其他对象一些信息，自省其本身获取请求信息，负责作为其他对象的代理或管理一个过程。  
与过程性编程语言中考虑以方法或函数的形式做这些事不同，消息的隐喻会要求你将对象看成执行的方法。对象不会是状态和行为的被动容器，而可以说是程序活动的代理。  
这种比喻其实非常有用。一个对象类似于在两种情况中的作用者：它既是整体程序设计中的特定角色，在这个角色当中，它可以独立于程序的其他部分。作为它们自己本身时，它们与其他的对象进行交互，但它是能够自包含的，并且在某个特定范围内能够扮演其本身的。就像一个演员在台上，他不能从剧本脱离，但他扮演的角色可以是多层次并丰富的。  
将对象看做演员这种想法很好的形容了面向对象编程——对象之间的沟通通过消息。你无需像函数那样调用方法，而是请求他执行其方法之一的对象发送消息。  
尽管可以花费一段时间适应，这种比喻引导了一种非常有效的观察函数和对象的方法。它将作用于特定数据的方法进行抽离并将动作进行浓缩。比如，在一个面向对象编程的接口中，start方法可能会初始化一个操作，archive方法可能归档一些信息，draw方法可能会创建一个图片。当操作被初始化，信息被封装，图片被绘制的时候，并不会显露方法名。不同的对象可能会以不同的方式执行这些方法。  
此外，方法灰机了抽象的动作。要调用这些动作中的其中一个，你需要将相关的方法汇集在一个对象当中进行聚合。这通过命名对象为消息的接收者就能够做到。你选做接收者的对象决定了哪些操作被初始化，哪些数据被归档或哪些图片被绘制。  
由于方法是属于对象的，它们只能够通过特定的接收者来调用（方法的拥有者和方法将要作用到的数据结构）。不同的接收者可以对于同一个方法有不同的实现。因此，不同的接收者在响应相同的消息时可以做不同的事。消息的结果不能仅从消息或方法名来判断，它同样取决于接收消息的对象。  
通过将消息（请求行为）从接收者（能够响应请求的方法的拥有者）分离开，消息隐喻完美地捕获了行为可以从它们的特定实现中抽象出来的想法。
## 类
一个程序可以拥有很多个同一种类型的对象。拿模拟用水量的程序举例，很可能有多个水龙头和管道对象以及一些电器用具和用户。同一种类型的对象被称作class的成员。一个类的所有成员都能够执行同样的方法以及匹配同样集合的实例变量。它们也共享同样的定义；同一种类型的对象只需一次定义。  
在这方面，对象与C的结构体类似。声明一个结构体定义了一种类型。举例来说，声明：  
...
定义了struct key类型。一旦定义，一旦定义，结构体的名称就能够被用来构建任意数量该类型的实例了：  

	struct key  a, b, c, d;
	struct key *p = malloc(sizeof(struct key) * MAXITEMS);

该声明是一种结构体的模板，但它不会创建程序能够使用的结构体。它需要花费一个额外的步骤来为一个实际的该类型的结构体分配内存，该步骤能够被重复任意多次。  
类似的，定义一个对象创建一类对象的模板。它定义了一类对象。模板能够被用来构建任意数量的类似对象——类的实例。举例来说，应当有一个单独定义的Faucet类。使用该定义，一个程序能够根据需要创建任意数量的Faucet实例。  
一个类的定义类似于一个结构体的定义，它对将要成为每个实例的一部分的数据元素进行了布局和安排。每个实例都有其自己实例变量的内存分配，它存储了实例的特定值。  
不过，一个类的定义与一个结构体的定义不同，它同样定义了类的成员的指定行为方法。每个实例都具有对为类所定义的方法的访问权限。两个拥有相同数据结构但不同方法的对象不会从属于同一个类。
### 模块性
对于一个C程序员来说，一个模块不只是一个包含了源代码的文件。将一个大型程序（或没那么大的）以一种合理的方式分解为不同的可管理的文件块。每块文件都能够独立工作并独立编译，当程序链接时可以与其他部分整合到一起。  
使用static存储类的标识符来限制文件。这种类型的模块是由文件系统定义的单元。它是一个源代码的容器，而非语言逻辑的集合。
### 重用性
使用面向对象编程的一个主要目的是为了让你的代码能够尽可能的复用——让其能够服务于很多不同的环境和应用程序——以便你能够避免重复实现，即使是有一点点不同，一些已经完成。  
可重用性受以下因素影响：  
...
这些因素不会仅作用于对象模型上。它们可以被用来判断任何代码的可重用性——标准C函数和类的定义等。比如高效和文档完备的函数比文档不全和不可靠的函数的可重用性好。  
不过，一个常见的对比会展示类的定义会让其本身的代码以一种函数不能做到的方式来重用。要让函数变得更能够重用，你可以做很多——比如，将数据以参数的形式进行传递而非设置全局变量。即使是这样，我们也会发现仅有一小部分函数能够被抽取为应用程序本身被设计时所需的那样。它们的可重用性从内部就被一三种方式限制了：  

* 函数名是全局的；每个函数都要有一个唯一的名称（除非声明为static）。当构建一个复杂的系统时，该命名的要求使得依赖库代码变得困难。编程接口会很难被理解和扩展，也不容易捕捉到显著的泛化。而另一方面，类，能够共享编程接口。当同样的命名规则被一次次使用时，大量的函数能被打包封装为一个很小的，容易理解的接口。
* 函数是一次性的从库中选取的。由程序员根据需要选取函数。而相对的，对象作为函数的集合，不是单独的方法和实例变量。它们提供集成的服务，所以面向对象的库的使用者不会陷入将自己的解决方案拼凑在一起的问题上。
* 函数是与特定类型的数据结构绑定的，并为特定程序所设计的。

一个对象的数据是被保护的，并不会被程序的其他部分接触到。因此方法可以信任其完整性。它能保证外部的存取不能够将数据设置为不合逻辑或不成立的状态。作为结果，一个对象的数据结构比传递给一个函数的要更可靠，方法也更能够依赖它。重用的方法因此更容易编写。
## 抽象的机制
在这一点上，对象被引入为包含更高层次抽象和应用程序中的连贯角色参与者的单元。不过，如果没有各种语言机制的支持，就无法使用这种方法。两种最重要的机制是封装和多态。  
封装将一个对象的实现与其接口进行了分离，多态会从给定的类的命名空间获取结果。以下段落依次讨论了这些机制。
### 封装性

### 多态性

## 继承
以已经理解的东西来解释新的事物是最终的方式。若你想要描述什么是“schooner（纵帆船）”，那么你的听众如果已经知道什么是“sailboat（帆船）”的话，将会对于理解很有帮助。若你要解释大键琴是如何工作的，那么你最好假设你的听众已经看过钢琴的构造或已经看过吉他演奏或至少对于一个乐器的原理比较熟悉。
### 类的层级
图3-3 一个继承等级  
...
# 结构化编程
面向对象程序有两种结构。一种可以在其类定义的继承层级中观察到。另一种在程序运行时消息传递的模式中展现。这些消息展现了对象连接的网络。  

* 继承链解释了对象是如何通过类型来发生联系的。举例来说，
...
# 结构化编程任务
面向对象编程不仅仅是以一种更好的方式组织程序结构的工作，它还能够帮助结构化的进行编程任务。  
当软件要做的事情越来越多，程序变得越来越大越来越复杂，随着管理任务产生的问题也在增长。会有更多的任务需要互相依赖，更多的人互相协作来进行构建。面向对象提供了方法来解决这种复杂性，不只是从设计角度，同样也在工作的组织上也有帮助。
## 协作
复杂的软件需要在单独工作的程序员之间有着高效的协作，使得他们之间所做的工作能够与其他人在做的契合。  
这一作用的规模和在同一时间同一地点同一个项目上工作的人数可能会影响到小组合作实现共同目标的能力。此外，协作通常受到时间，位置和组织的限制：  

* 在代码被写完后，必须被维护，改进和使用的足够久。同一个项目的协作程序员可能不会在同一时间工作，所以他们可能不会再同一位置进行讨论以及保持实现细节等信息的同步。
* 即使程序员在同一个项目同一时间共同工作，他们也不会在同一个位置聚在一起。这也同样妨碍了他们能够合作的紧密度。
* 在不同组织不同等级不同安排的程序员通常都要在项目上进行协作。跨越组织的障碍进行沟通通常不太容易做到。

这些问题的答案必须从程序的设计和编写的方式中发展出来。它不能从外部强加于层次化管理结构和严格的权限级别。这些往往对于人的创造力有损害。相反，协作必须在工作中建立。  
这就是面向对象编程技术能够帮到的地方了。比如，面向对象代码的可重用性意味着程序员能够高效的协作，即使他们在不同的项目，不同的时间或不同的组织形式进行工作，他们都可以在库中共享代码。这种类型的协作可以轻松的减轻困难的任务，使看似不可能的项目得以实现。
## 面向对象工程的组织
面向对象编程能够以有益于协作的方式重新构建程序任务。它减少了底层实现细节需要的协作，而在高层次的协作上起到了促进结构作用。几乎所有的对象模型的功能，从大规模设计能力到增加代码的重用性，都对人们协同工作的方式起到了影响。  
### 大规模设计
在高层次的抽象级别进行程序设计时，考虑分工更容易。它能够匹配逻辑上程序的划分；项目的组织方式可以从设计中发展出来。  
使用面向对象来设计更容易保持通用的目标，而非在实现中迷失，并且跟容易让每个人都看到它们设计的部分是如何适应整体的。所以它们的协作效应能够更好的达到目标。
### 从实现中抽离接口
面向对象程序各个组件之间的连接在设计的早期就已经解决了。至少在开发的初始化阶段，在实现开始之前，它们就已经被定义好了。  
在实现期间，只有接口需要协调，并且大部分都自然的脱离了设计。由于每个类都对其实现进行了封装并有其自己的命名空间，所以无需协调实现细节。协调所需的越少，协作就越简单。
### 将工作分解成模块
面向对象编程的模块化意思是将一个大的程序的逻辑组件进行分拆实现。不同的人能够在不同的类当中进行工作。每部分任务的实现都独立于其他部分。  
模块化是有收益的，不只是对于实现的组织上，而且对后续问题的解决也有益。由于实现是包含在类之内的，那么呈现出的问题也是独立的。当在一个定义良好的程序中定位问题时也会容易的多。  
将责任分摊至类也意味着每部分都能够独立运行。类能够定期的优化其性能以及使用最新的技术。这些更新无需让程序的其他部分进行调整。只要一个对象的接口没有改变，其实现的改进随时都能够部署。
### 保证接口的简单
面向对象程序的多态性同时也产出了更简单的编程接口，因为同样的命名和习惯将会被重用到任意数量的类当中。其产出无需了解更多，对于整体系统是如何运作的更容易理解，而合作和协作的方式更为简单。
### 动态的做出决定
由于面向对象编程是在运行时动态的做出决策的，这样，在编译时（在源代码中）就需要较少的支持来使得两块代码共同协作了。因此，对于定位和错误就更少了。
### 继承遗留的代码
继承是一种重用代码的方式。若你能够将定制化的类定义的更为通用，你的编程工作量将会大大降低。设计也会简化，因为继承层次结构列出了不同实现级别之间的关系，并使其更容易理解。  
继承同样增加了代码的可重用性和可靠性。位于父类的代码会被其子类所检测。你在库中找到的泛型类将会被其他的程序员为其他应用程序所编写的子类所检测。
### 复用可测试的代码
你从其他的软件能够借用整合到你的程序中的东西越多，你需要自己做的事情就越少。有很多的软件都是从面向对象编程环境中借用的代码，因为代码是可以复用的。程序员在不同组织不同位置之间的协作得以增强，而每个项目的压力在减少。  
从面向对象库中获取的类和框架能够大量的充实你的程序。举例来说，当你的软件使用Apple提供的软件框架时，你就是在有效的和Apple的程序员进行协作；你负责将你的程序的很大的一部分交给他们来开发。你只需将精力集中在你需要做到最好的部分，其余的任务交给库的开发人员即可。你的项目将会更快成形，更快的编译，在自己的这部分将会较少的遇到协作方面的挑战。  
面向对象代码的可重用性的增长同样也增加了其可依赖性。从库中取的类可能已经适配了各种应用程序和情况。代码被使用的越多，遇到的问题和被解决的就越多。你的程序的bug很可能已经被跟踪或排除。