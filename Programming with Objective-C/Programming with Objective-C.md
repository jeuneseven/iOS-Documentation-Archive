...
# 关于OC
在为OS X和iOS编写软件时，OC是一门主要使用的编程语言。这是一个C语言的超集，提供面向对象的功能以及动态运行时能力。OC继承了C语言的语法、基本数据类型以及流控制语句，并且添加了典型的类和方法语法。它还为对象的管理提供了语言级别的支持，并且在动态类型绑定时提供了对象字面量，将很多职责推迟到了运行时。
## 概览
...
OC使用协议来定义一组相关的方法，例如方法由一个对象的delegate调起，代理方法有可选或者必须的。任何类都可以声明它遵守了一个协议，这意味着它必须实现所有在协议中要求的必须实现的方法。  
`相关章节：使用协议`

### 值和集合类通常表现为OC的对象
通常在OC中使用Cocoa或Cocoa Touch的类来表达一个值。NSString类表达一组字符串，NSNumber类是为多种不同类型的数字准备的，例如整形或者浮点型，NSValue类为其他的值比如C的结构体准备的。你还可以使用各种由C语言定义的基本数据类型，例如int，float或者char。  
集合通常表现为一种集合类的实例，例如NSArray, NSSet, 或者NSDictionary，这些通常用在收集其他OC对象上。  
`相关章节：值和集合`

### Block简化了常用任务
Block是一个语言级别的功能，C、OC以及C++当中都引入了该功能，它代表了一组工作集合；它将一块代码连同捕获的状态进行封装，这很像其他编程语言当中的闭包功能。block通常用来简化日常的任务，例如集合的枚举、排序和测试等。它还简化了为并行或者异步执行而设计的GCD的预定任务。  
`相关章节：使用Block`
...
对于编译器而言，它是和sayHello函数完全不同的两个函数。  
通常来讲，函数名使用小写字母开头。OC当中通常使用更具有描述性的名称为函数命名，而不是以往在C中看到的那样。如果一个函数名包含多个单词的话，请使用驼峰命名（每个新单词的首字母大写）使它们更便于阅读。  
请注意，空格在OC当中也是很灵活的。通常每行代码的缩进使用tab或者空格键，并且你会经常看到左大括号在一个单独行当中，类似这样：  
> -(void)sayHello  
> {  
>    NSLog(@"Hello, World!");  
> }

Xcode是Apple为OS X和iOS开发提供的集成开发环境（IDE），它会根据定制的用户习惯自动为你的代码缩进。参见“在Xcode工作空间中改变缩进和tab宽度”相关信息。  
...
因为app是一个多线程的，属性很有可能在你测试它或者函数调用的时候就已经释放了，这会导致测试毫无用处。所以，你需要声明一个强引用的局部变量来持有它，类似这样：  
...
这样的话，你就可以在运行的时候使用自己的方法了。冲突造成的歧义就不存在了，因为你的代码会是这样的：  
...
## 思考一下其他的类的定制化的方法
分类和延展为已经存在的类添加行为提供了很方便的方式，但有的时候这并不是最好的选择。  
...
很多种情况下，例如哪个函数在被发送消息的时候该被调用，不是在编译时期决定的，而是在app运行时决定的。OC不是一门编译至机器代码的编程语言。它需要一套运行时系统来执行代码。  
...
为了表明你自己的协议遵循NSObject协议，你应该将任何遵循自定义协议的对象实现NSObject协议的方法。由于你基本使用的都是NSObject类的子类，所以你无需担心提供自己的类的关于NSObject协议的实现。不过遵守这些协议是很有用的，这在之前描述过。  
...
@end
	...
	提示：如果你发现你在一个类当中遵守了大量的协议，这可能意味着你需要通过分解必要的行为到更小的多个类当中来重构一个过度复杂的类，每个类里面定义的行为都更加清晰。  
...
# 值和集合
尽管OC是一个面向对象的编程语言，它还是C的一个超集，意思是你可以在OC当中使用任何标准C的基本（非对象）类型，例如int, float 和 char等类型。此外，还有一些基本类型在Cocoa 和 Cocoa Touch的app当中使用，例如NSInteger, NSUInteger 和 CGFloat等，不同的定义取决于目标框架的不同。  
基本类型通常在你不需要一个对象表现一个值的时候使用（或者关联开支）。而字符串通常由NSString类的实例表示，数字形的值通常保存在标准的局部变量或者属性当中。  
...
类似NSInteger 和 NSUInteger这种类型是根据不同的框架来定义的。当在32位的环境当中构建时（例如iOS），它们分别表示32位的有符号和无符号整型变量；当在64位环境上构建时（例如现代的OS X运行时），它们分别表示64位的有符号和无符号整型变量。  
...
一旦你创建了NSNumber对象，你就可以使用存取方法来访问它的基本类型值：  
...
通过NSValue对象来表达自定义的结构体也是可以的。如果你有特殊的需要，使用一个C的结构体（而不是一个OC的对象）来保存信息的话，类似这样：  
> typedef struct {  
    int i;  
    float f;  
...
![](https://developer.apple.com/library/content/documentation/Cocoa/Conceptual/ProgrammingWithObjectiveC/Art/dictionaryofobjects.png)  
	...
	如果你想使用KVC的话，这在“KVC编程指南”中有相关描述，你必须使用字符串作为字典中对象的key。
...
### 使用NSNull来表示nil
你是无法将nil加入到本段当中提到的集合类当中的，因为nil在OC当中意味着“没有对象”。如果你需要在集合当中表达“没有对象”的概念的话，你应该用到NSNull类：  
...
## 使用高效的集合枚举技术
OC以及Cocoa 或 Cocoa Touch提供了大量的方法来枚举集合中的内容。尽管可以使用传统的C的for循环来迭代访问内容，类似这样：  
...
在快速枚举的时候，你是不可以改变一个集合的，即使该集合是可变类型的。如果你试图在循环中添加或删除一个元素的话，你会在运行时得到一个异常。
### 大部分集合同样支持枚举对象
...
如果你对C的函数指针不太熟悉的话，这个语法可能看起来有点不一样。这个例子声明了一个变量叫做simpleBlock，它指向一个不带任何参数并且没有返回值的block，这意味着变量可以被上面的block代码块赋值，类似这样：  
> simpleBlock = ^{  
        NSLog(@"This is a block");  
    };

这跟其他的变量赋值一样，所以该语句的末尾必须跟着一个分号。你还可以把变量的声明和赋值写到一起：  
> void (^simpleBlock)(void) = ^{   
        NSLog(@"This is a block");  
    };
...
这个例子调用了一个方法来展示进度条，然后创建了一个任务告诉他要开始了。回调block指定了一旦任务完成就要执行的代码；在这个例子中，调用隐藏进度条的方法是很简单的。注意，这个回调的block持有了self，以便在需要调用hideProgressIndicator方法的时候进行调用。当持有self的时候需要特别关注，因为这很容易引起强引用循环，这在之后的“在持有self时避免强引用循环”章节中有相关描述。  
在代码的可读性方面，block在一个地方将任务开始前和结束后都简单的表达了出来，避免了通过代理方法来跟踪到底发生了什么。  
...
一个比较好的习惯是在一个方法当中只使用一个block。如果该方法需要非block类型的参数的话，block应该放置在最后：  
...
### 对象可以使用属性来跟踪Block
定义一个属性来持有block的语法和定义一个block变量的语法很相似：  
> @interface XYZObject : NSObject  
@property (copy) void (^blockProperty)(void);  
@end

	注意：你需要指定copy作为属性的关键字，因为block需要被拷贝才能持有它捕获的原有范围内之外的状态。这在你使用ARC的时候无需担心，因为这是自动发生的，但是通过属性来展示最终的结果行为是很好的一种方式。更多信息，参考“block编程话题”。

一个block属性被设置或者调用就跟其他的block变量一样：  
> self.blockProperty = ^{  
        ...  
    };  
    self.blockProperty();

同样的，你也可以使用类型定义来为block属性进行声明，类似这样：  
> typedef void (^XYZSimpleBlock)(void);  
@interface XYZObject : NSObject  
...
这提供了更简单的方法来枚举键值对。
## Block能够简化并发任务
block代表了一组独立的工作任务，它将可执行的代码和持有上下文的状态结合了起来。这让它成为了OS X和iOS上处理并发任务的异步调用的理想选择。你无需关心底层的线程机制是如何运作的，你只需要使用block定义你自己的任务，然后让系统在处理器资源可用的时候来执行就可以了。  
...
# 处理error
几乎所有的app都会遇到error。有的情况下，这些error你是无法控制的，比如运行到了磁盘之外，或者没有网络连接。有的error可以恢复，比如无效的用户输入。并且，即使开发者是力求完美的，一些开发者造成的临时的error还是会发生的。  
...
### 尽可能的恢复或者展示error给用户
最佳的用户体验是你的app显式的从error恢复。举个例子，如果你发起了一个请求给远端服务器，你可能需要再次请求一次，并且更换服务器。或者，你需要用户提供有效的用户名和密码再次请求一次。  
...
# 惯例
当你使用系统的框架类的时候，你会注意到，OC的代码是非常容易阅读的。比起你用到的C的函数或者C标准库而言，OC的类名和方法名更具有表意性，名称的命名用了多个单词组成，并且使用驼峰命名法。当你编写你自己的代码的时候，你应该遵循同样的Cocoa 和 Cocoa Touch的命名方式，使得你的代码更为可读，不论是你还是其他的使用OC的程序员，在开发你的工程的时候，都能够保证代码库的一致性。  
此外，很多OC的类库需要你严格遵守命名规则为了能够让app的机制正确的运行。举个例子，存取方法必需遵循KVC或者KVO。  
本章覆盖了一些最通用的在Cocoa 和 Cocoa Touch上编码的惯例，解释了什么时候一些名称会在整个app当中是唯一的，包括它关联的类库。
## 一些名字必须是在你的app当中唯一的
每当你创建一个新的类型、符号或者ID的时候，你都应该首先考虑在该名称的使用范围内的唯一性。有的时候，这个范围会是整个app，包括它相关的类库；有的时候这个范围限于它包含的类或者仅仅是一部分代码块当中。
### 类名必须在整个App当中都是唯一的
OC的类名不仅仅在你编写的工程当中，而且是相关的类库或者bundles当中都必须是唯一的。举个例子，你应该避免使用类似ViewController 或者 TextParser这种比较通用的名字，因为很有可能在你的app当中有个类库使用了同样的名字。  
为了保持类名的唯一性，惯例是在所有的类当中都使用前缀。你应该注意到了， Cocoa 和 Cocoa Touch类名都使用了NS或者UI作为前缀。两个字母的前缀是为Apple的系统类库保留的。你使用 Cocoa 和 Cocoa Touch越多，你就会遇到越多的带有前缀的类库：  

| 前缀  | 系统类库  |
|:------------- |:---------------:|
| NS | 基础框架（OS X和iOS）和Application Kit (OS X) |
| UI | UIKit (iOS) |
| AB | Address Book |
| CA | Core Animation |
| CI | Core Image |

你自己的类应当使用三个字母的前缀。这应该和你的公司名或者你的app名称相关，或者是你的app当中的一个特殊组件。举个例子，如果你的公司叫做Whispering Oak，你开发的游戏叫做Zebra Surprise， 你可能选择WZS 或者 WOZ作为你的类的前缀。  
你应该使用名词来给你的类取名，这能够让它更明显的表示它到底想表达什么，比如Cocoa 和 Cocoa Touch的几个类：  

| NSWindow  | CAAnimation  | NSWindowController | NSManagedObjectContext |
|:------------- |:---------------:|:---------------:|:---------------:|

如果在类名当中需要多个单词的话，你应该遵循驼峰命名法，每个单词的首字母大写。
### 方法名最好具有丰富的表达性并且在类当中唯一
一旦你为一个类选择了一个唯一的名字，你声明的方法只需要在你的类当中保持唯一性就可以了。举个例子，你在其他类当中，是可以使用相同名称的函数的。无论是重写子类方法或者是利用多态性。在多个类当中执行相同任务的方法必需具有相同的名称、返回值和参数类型。  
方法名无需有前缀，并且应该以小写字母开头；驼峰命名法同样适用，比如这些NSString类当中的方法：  

| length  | characterAtIndex:  | lengthOfBytesUsingEncoding: | 
|:------------- |:---------------:|:---------------:|

如果一个方法带有一个或者多个参数的话，函数名必需指明每个参数：  

| substringFromIndex: | writeToURL:atomically:encoding:error:  | enumerateSubstringsInRange:options:usingBlock: | 
|:------------- |:---------------:|:---------------:|

方法的第一部分必需支持方法的最初的目的或者调用方法的结果。举例来说，如果一个方法有返回值，第一个单词通常应该指明将会有返回值，比如上面提到的length, character... 和 substring...等。如果你想表明一些有关返回值的很重要的信息的话，你可以使用多个单词，比如NSString 类中的 mutableCopy, capitalizedString 或者 lastPathComponent方法。如果方法执行了某种行为，比如写入磁盘或者枚举内容的话，第一个单词应该指明该行为，比如上面提到的write... 和 enumerate...方法。  
如果一个方法包含一个在error发生的时候要被设置的error指针的话，这个参数应该在函数的最后。如果一个函数包含一个block的话，block参数也应该在最后，并且它应该让函数的可读性更强。同样的原因，最好避免一个函数携带多个block参数。  
清晰并且简单的函数名是很重要的。清晰不代表冗长，简明不代表结果太过简单，所以应该平衡这两点：  

| stringAfterFindingAndReplacingAllOccurrencesOfThisString:withThisString: | 太过冗长 | 
|:------------- |:---------------:|
| strReplacingStr:str: | 太过简单 | 
| stringByReplacingOccurrencesOfString:withString: | 刚刚好 | 

你应该尽量避免在函数名当中使用缩写，除非你能够确定该函数名是能够在多种语言和环境当中都能够通用的。常用的缩写在“可接受的缩写和缩略字母”中已经列出。
#### 在类库的分类的方法当中应该始终使用前缀
当你使用分类来给系统类库添加方法的时候，你应该在函数名前添加前缀，这在“避免分类方法冲突”中提到过了。
### 局部变量在同一个作用域范围内必须唯一
...
## 一些方法名必须遵从惯例
考虑到唯一性，一些比较重要的函数类型也要严格遵循惯例。这些惯例被OC底层的机制所使用，编译器和运行时，以及Cocoa 和 Cocoa Touch的一些行为。
### 存取器方法必须遵从惯例 
当你使用@property语法来为一个对象声明属性的时候（这在“封装数据”当中提到过），编译器会自动给你合成相关的存取方法（除非你另外指明）。如果你出于某种原因想要提供你自己的存取方法的话，你一定要确认你使用的是正确的函数名，这样你的函数才能够使用点语法被调用到。  
除非特殊指明，一个getter方法应该和属性名称一样。如果一个属性叫做firstName的话，那么他的getter方法也应该叫firstName。唯一的例外是布尔类型的属性，每个getter方法都应该以is开头。比如一个属性叫做paused，那么它的getter方法应该叫做isPaused。  
一个属性的setter方法应该是用这种格式：setPropertyName:。比如一个属性叫做firstName，那么它的setter方法应该叫做setFirstName:，如果是布尔类型的属性paused，那么它的setter方法应该叫做setPaused:。  
尽管@property语法允许你指定自己的不同的存取方法名，你也应该仅仅在布尔类型的属性这种情况下才使用。应该遵循一些基本的惯例，否则类似KVC（一种通过使用valueForKey: 和 setValue:forKey:对属性进行获取或者设置的能力）这种技术是不会起作用的。更多关于KVC的相关信息，参见“KVC编程指南”。
### 对象的初始化方法必须遵从惯例
就像你在之前章节当中看到的那样，有多种方法能够创建一个类的实例对象。你可以使用alloc init方法，类似这样：  
>  NSMutableArray *array = [[NSMutableArray alloc] init];

或者使用new这种比较方便的方法来代替alloc init：  
> NSMutableArray *array = [NSMutableArray new];

有些类还提供类工厂方法：  
> NSMutableArray *array = [NSMutableArray array];

类的工厂方法应该始终以它要创建的类的名字开始（没有前缀），子类的已经存在的工厂方法除外。  
举例来说，NSArray类，工厂方法叫做array。NSMutableArray类无需定义任何它自己的工厂方法，所以它的可变数组依旧以array开头。  
在OC当中有着很多的内存管理规则，编译器将会确保对象的生命周期根据需求存活的足够长。尽管你无需关心这些规则，编译器根据生成的函数名称来判断它应该遵从什么规则。通过工厂方法创建的对象和通过alloc init或者new创建的对象有着些微的不同，因为使用了自动释放池。更多关于自动释放池以及内存管理的相关信息，参见“高级内存管理编程指南”。