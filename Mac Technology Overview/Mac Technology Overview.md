...
应用是最常见的 Mac 软件类型，不过你也可以创建很多其他类型的软件。以下章节介绍了你可以为 Mac 平台创建的软件产品的范围以及当你考虑这么做的时候的一些建议。

## 应用

应用应该是为 Mac 创建的主要类型的软件。使用 Cocoa 来构建新的 Mac 应用。要了解更多的 Cocoa 功能和框架，参见《Cocoa 应用层》。  
通常来讲，有三种基本类型的 Mac 应用：  
...
由于 OS X 应用能够比 iOS 应用展示的数据内容更多，你可以扩展你的数据模型
