...
* 静态cell。使用静态cell来设计一个列表，是通过固定数量的行来控制，每行都有其自己的布局。当你知道列表在设计时是什么样就可以使用静态cell，而不用管其展示的特定信息。
* 动态类型。使用动态类型的来设计一个cell是将其用作列表中其他cell的模板。当一个列表中的多个cell都应使用相同布局来展示信息时，就应使用动态类型。动态类型内容是在运行时被数据源所管理的，并且拥有任意数量的cell。
